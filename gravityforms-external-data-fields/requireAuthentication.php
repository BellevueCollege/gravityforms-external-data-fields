<?php
    ...
    class requireAuthentication {
        ...
        static function isAuthenticated () {
            ...
            return ( !( is_null ( $_SESSION[ requireAuthentication::SESSION_USERNAME ] ) ) && ( $_SESSION[ requireAuthentication::SESSION_USERNAME ] != "" ) );
        }
        ...
        private function hasShortcode () {
            ...
            foreach ( $codes as $code ) {
                $hasShortcode = preg_match_all ( '/' . $pattern . '/s', $post->post_content, $matches ) && array_key_exists ( 2, $matches ) && in_array ( $code, $matches[ 2 ] );
                if ( $hasShortcode ) {
                    /*
        ...
        private function ssoInitialize () {
            ...
            if ( !class_exists ( 'phpCAS' ) ) {
                ...
                if ( !empty( $cas_path ) ) {
                    ...
                    try {
                        phpCAS::client ( CAS_VERSION_2_0, gf_external_data_fields_config::$ssoServer, gf_external_data_fields_config::$ssoPort, gf_external_data_fields_config::$ssoPath );
